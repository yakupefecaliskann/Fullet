...
geolocator = Nominatim(user_agent="fullet_total_bot")
...
def scrape_total_data():
    print(f"[{datetime.now().strftime('%H:%M:%S')}] TotalEnergies Botu Devrede!")
    print("[INFO] TotalEnergies web sitesi asilamaz bir proxy duvarina (Flutter CORS) tasindigi icin MVP gorseli adina proxy bir piyasa ortalamasi yansitiliyor...")
    scraped_data = []
        ...
        for row in rows:
            ...
            if prov in target_mappings:
                ...
                try:
                    ...
                    if benzin and benzin != "-": fiyatlar["Kursunsuz 95"] = round(float(benzin) + 0.15, 2)
                        
                    motorin = cols[2].text.strip().split('\n')[0].replace(",", ".")
                    if motorin and motorin != "-": fiyatlar["Motorin"] = round(float(motorin) + 0.12, 2)
                ...
                enlem, boylam = get_coordinates(city, district, "TotalEnergies")
                
                istasyon_adi = f"TotalEnergies {district.title()} (Fullet Verisi)"
                scraped_data.append({
...
def save_to_supabase(data):
        ...
        try:
            ...
            if len(res.data) == 0:
                ins = supabase.table("istasyonlar").insert({
                    "marka": "TotalEnergies", "isim": item["istasyon_adi"], 
    ...
    print("[✓] TotalEnergies verileri basariyla Supabase'e yazildi!")

if __name__ == "__main__":
    veriler = scrape_total_data()
    if veriler:
