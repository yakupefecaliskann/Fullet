...
def scrape_data():
    print(f"[{datetime.now().strftime('%H:%M:%S')}] Petrol Ofisi bot started.")
    scraped_data = []
    ...
    try:
        ...
        if table is None:
            print("[WARN] Petrol Ofisi: fiyat tablosu bulunamadı.")
            return scraped_data

        # Sabit kolon indeksi yerine başlık metninden çözüm (yol haritası S1-2):
        # kaynak tabloya kolon eklendiğinde sessizce yanlış ürün okunmasın.
        column_map = resolve_fuel_columns(_header_cells(table))
        print(f"[INFO] Petrol Ofisi kolon eşlemesi: {describe_column_map(column_map)}")
        if not column_map:
            print("[WARN] Petrol Ofisi: yakıt kolonu eşleşmedi, kazıma durduruldu.")
            return scraped_data
        ...
        print(f"[WARN] Petrol Ofisi scrape failed: {exc}")
...
if __name__ == "__main__":
    ...
    data = scrape_data()
    summary = save_to_supabase(data, default_brand="Petrol Ofisi")
    print(f"[OK] Petrol Ofisi finished in {(datetime.now() - start_time).total_seconds():.1f}s.")
    raise SystemExit(finish_bot_run("po_bot.py", scraped=len(data), summary=summary))
